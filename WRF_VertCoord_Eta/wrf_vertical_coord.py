...
def standard_atm(z,pb,hb):
   ...
   #g0=9.80665 # m/s^2    - standard gravity acceleration
   g0=9.81 # m/s^2        - standard gravity acceleration in WRF
...
def stretch_coefficient(amp,nz):
    ...
    x=np.arange(nzc2-nzc1)
 
    pi=np.arccos(-1.)

    x=pi*x/max(x)

    cf[nzc1:nzc2] = 1. + amp * 0.5*(np.cos(2.*x+pi)+1.) 
...
for k in range(nz):
    print ('{0:6d},{1:12.5f},{2:14.5f},{3:14.5f}'.format(k,z[k],dzk[k],p[k]))

eta=create_eta(z,pb,hb)

kk = [l+1 for l in range(int((nz-4)/4))]
